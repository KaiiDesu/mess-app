...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('messages-container').addEventListener('touchend', () => clearTimeout(pressTimer));

  document.getElementById('messages-container').addEventListener('mousedown', e => {
    if (e.target.closest('.bubble')) {
      pressTimer = setTimeout(() => document.getElementById('reaction-picker').classList.add('show'), 500);
    }
  });
  document.getElementById('messages-container').addEventListener('mouseup', () => clearTimeout(pressTimer));
